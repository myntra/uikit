...
describe('Masked Input with mask pattern', () => {
  ...
  const wrapper = mount(<InputMasked pattern={'dL"ABC"lAa*'} onChange={handleChange} includeMaskChars={true} />)
...
describe('Masked Input that excludes mask character', () => {
  ...
  const wrapper = mount(
    <InputMasked pattern={'w*a"ABC"lA"$"'} placeholder={'xxxABCxx$'} onChange={handleChange} value={initValue} />
  )
  ...
  it('should not remove mask on backspace if pattern starts with a mask', () => {
    ...
    mount(<InputMasked pattern={'"$."dd'} includeMaskChars={false} onChange={handleChange} />)
      .find('.masked-input')
