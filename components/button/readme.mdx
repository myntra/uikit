...
<div style={{
  ...
  <Button type="link" onClick={() => setDirection('up')}>Up</Button>
  <Button type="link" onClick={() => setDirection('down')}>Down</Button>
  <Button type="link" onClick={() => setDirection('left')}>Left</Button>
  <Button type="link" onClick={() => setDirection('right')}>Right</Button>
  <Button.FAB icon="search" direction={direction} style={{
    position: 'absolute',
    top: '50%',
    left: '50%',
    ...
    <Button icon="angle-down" label="Angle Down" />
  </Button.FAB>
</div>
```