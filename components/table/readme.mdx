...
<div style={{ height: 300, minWidth: 600, maxWidth: '90vw' }}>
 <Table virtualized data={data}>
  ...
  <Table.Column key="firstName" label="First Name">
    <Table.Column key="realFirstName" label="First Name" minWidth={120}>
  ...
  {Array(997).fill(0).map((_, index) => (
    <Table.Column key={'lastName' + index} label={`Column (${index + 4})`} minWidth={100}>
      {({ data }) => <div>{data.lastName}</div>}
    </Table.Column>
  ))}
...
```jsx preview
const [data] = useState(Array(40).fill(null).map((_, value) => ({ value })));
