...
<Table
  ...
  renderHeaderRow={({ index, data, children, ...props }) => (
    <Table.TR {...props}>
      {/* header cells */}
    </Table.TR>
  )}
...
>
  ...
  <Table.Column
    ...
    label="ID" // Text for header
    renderLabel={() => <span>ID</span>} // Renderer to customize appearance of column header
